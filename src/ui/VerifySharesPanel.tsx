...
export function VerifySharesPanel({ overview, aggregate, shares, selectedElection }: Props) {
  ...
  return (
    <div className="vpInline" role="region" aria-label="Decryption shares verification guide">
      <div className="vpInlineHdr">
        <div>
          <div className="vpHeaderLabel">RE-VERIFY DECRYPTION SHARES LOCALLY</div>
          <div className="vpHeaderSub">{shares.length} keyper{shares.length !== 1 ? "s" : ""} · {overview.config.numCandidates} candidates each</div>
        </div>
      </div>

      <div className="vpBody">
        <p className="vpIntro">
          Confirm that each keyper's decryption share is cryptographically bound to their committee
          public key. A DLEQ proof is published alongside every share — verify it yourself to rule
          out fabricated or corrupted shares.
        </p>

        <div className="vpStep">
          <div className="vpStepNum">1</div>
          <div className="vpStepContent">
            <div className="vpStepTitle">Download the shares fixture</div>
            <p className="vpStepDesc">
              Contains the on-chain aggregate, all keyper decryption shares with their DLEQ proofs,
              and the committee public keys.
            </p>
        ...
        <div className="vpSection">
          ...
          <div className="vpCheckList">
            <div className="vpCheckItem">
              <div className="vpCheckName">DLEQ proof per share</div>
              <div className="vpCheckDesc">
                Each share σ_i = s_i · C₁ (on the aggregate ciphertext) is accompanied by a discrete-log
                equality proof showing the same secret s_i produced σ_i and the keyper&apos;s committee
                public key (G₂). This prevents
                a corrupted or fabricated share from passing undetected.
              </div>
            </div>
            <div className="vpCheckItem">
              <div className="vpCheckName">Per-keyper, per-candidate</div>
              <div className="vpCheckDesc">
                Every keyper must submit one valid share per candidate ciphertext.
                All shares are checked independently — a single bad share is flagged.
              </div>
            </div>
          </div>
        </div>
